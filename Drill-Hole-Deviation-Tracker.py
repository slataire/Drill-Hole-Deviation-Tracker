...
# New helper that parses any dataframe with flexible headers
...
def parse_csv_flexible(file):
    df = pd.read_csv(file)
    return parse_df_flexible(df)

# New excel parser
def parse_excel_flexible(xls, sheet_name):
...
def ensure_zero_station(stations, seed_from="first_survey", plan_az0=None, plan_dip0=None, manual_az0=None, manual_dip0=None):
    # If the smallest MD > 0, insert MD=0 using first survey, planned start, or manual collar
    if not stations:
        return stations
    stations = sorted(stations, key=lambda d: float(d["MD"]))
    ...
    elif seed_from == "manual" and manual_az0 is not None and manual_dip0 is not None:
        az0, dip0 = float(manual_az0), float(manual_dip0)
    else:
...
# Manual collar inputs for the actual hole
colMC1, colMC2 = st.columns(2)
with colMC1:
    actual_collar_az = st.number_input("Actual collar azimuth deg", value=90.0, step=1.0)
with colMC2:
    actual_collar_dip = st.number_input("Actual collar dip-from-horizontal deg (negative down)", value=-60.0, step=1.0)

if method == "CSV or Excel upload":
    file = st.file_uploader("Upload CSV or Excel (MD, Azimuth, Angle). Example headers accepted: MD, Azimuth, Angle or Measured Depth, Azi, Dip", type=["csv", "xlsx", "xls"])
    flip_sign = st.checkbox("Input angle is positive-down - flip sign to negative-down", value=False)
    ...
    if file is not None:
        ...
        if name.endswith(".csv"):
            ...
            df_in = parse_excel_flexible(xls, sheet)
        ...
        if not flip_sign and len(df_in) > 0 and df_in["Angle"].median() > 0:
            df_in["Angle"] = -df_in["Angle"]
            st.info("Detected positive-down dips in input. Converted to negative-down.")
...
# enforce MD=0
seed_mode = st.selectbox("If surveys start after 0 m, seed MD=0 using", options=["first_survey", "planned", "manual"], index=0)
actual_stations_base = ensure_zero_station(
    actual_stations_base,
    seed_from=seed_mode,
    plan_az0=plan_az0,
    plan_dip0=plan_dip0,
    manual_az0=actual_collar_az,
    manual_dip0=actual_collar_dip
)
...
# plane sized to cover entire axes cube
...
st.caption("Angles are dip-from-horizontal. Negative values point down. Target plane is positioned by downhole MD on the planned hole. If input does not start at 0 m, a 0 m station is inserted.")
