...
def apply_cfg_to_session(cfg):
    ...
    _plan_len_now = float(st.session_state.get("plan_len", 500.0))
    _target_md = cfg.get("plane", {}).get("target_md", st.session_state.get("target_md", max(0.0, _plan_len_now - 50.0)))
...
def ensure_zero_station(stations, use_planned=False, plan_az0=None, plan_dip0=None, actual_az0=None, actual_dip0=None):
    ...
    - If use_planned is True: use planned collar az/dip.
    - Else: use provided actual collar az/dip if given, otherwise fall back to first survey's az/dip.
    """
    if not stations:
        return stations
    stations = sorted(stations, key=lambda d: float(d["MD"]))
    ...
    elif actual_az0 is not None and actual_dip0 is not None:
        az0, dip0 = float(actual_az0), float(actual_dip0)
    else:
...
method = st.radio(
    "Provide surveys via",
    _method_options,
    index=_method_options.index(st.session_state.get("survey_method", "CSV or Excel upload")),
    horizontal=True,
    key="survey_method",
)
...
if method == "CSV or Excel upload":
    ...
    if "loaded_surveys_df" in st.session_state:
        manual_df = st.session_state["loaded_surveys_df"].copy()
        manual_df = manual_df.rename(columns={"Angle": "Dip (Negative is Down)"})
        if "Active" not in manual_df.columns:
            manual_df["Active"] = True
    else:
...
actual_stations_base = ensure_zero_station(
    ...
    plan_az0=st.session_state.plan_az0,
    plan_dip0=st.session_state.plan_dip0,
...
if plane_enabled:
    ...
    with colP3:
        # Clamp the displayed default against the current planned length so a
        # reloaded target_md can never exceed max_value.
        default_md = float(min(float(st.session_state.target_md), float(st.session_state.plan_len)))
...
cfg_dict = {
    ...
    "flip_sign": bool(st.session_state.get("flip_sign", False)),
    "survey_method": str(st.session_state.get("survey_method", "CSV or Excel upload")),
