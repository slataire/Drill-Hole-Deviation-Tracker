...
def make_planned_stations(length_m, step_m, az0, dip0, lift_per100, drift_per100):
    # Positive lift shallows dip toward 0. Negative dip is down.
    md = 0.0
    az = wrap_az(az0)
...
def derive_lift_drift_last3(stations):
    ...
    # Positive lift means dip increases toward 0 with MD.
...
# planned inputs - defaults now +2 lift and +1 drift
...
if method == "Excel upload":
    ...
    if file is not None:
        try:
            # Let pandas choose engine if available. If engine is missing, show a clean hint.
            ...
            st.error(f"Failed to read Excel: {e}. If this is a .xlsx file, install openpyxl. For .xls, install xlrd.")
...
# auto or manual sign flip for dips
if not df_in.empty:
    if flip_sign:
...
if force_manual_collar:
    ...
    # if not forcing, still ensure a 0 station exists, using file start as fallback
    if actual_stations_base and actual_stations_base[0]["MD"] > 1e-9:
...
# ---------- 3D view with enlarged bounds and plane ----------
st.markdown("### 3D view")
...
st.caption("Angles are dip-from-horizontal. Negative values point down. Positive lift shallows the dip toward 0. Target plane is positioned by downhole MD on the planned hole. If your file does not start at 0 m, a 0 m station is inserted or overridden by the manual collar, depending on the toggle.")
